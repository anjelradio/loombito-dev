...
import BoletinesStudentItem from "./BoletinesStudentItem";
...
export default function BoletinesCourseItem({ schoolId, course }: Props) {
  const [open, setOpen] = useState(false);
  const [students, setStudents] = useState<Student[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    // Solo hacemos fetch si abrimos el sheet y la lista de estudiantes esta vacia
    if (open && students.length === 0) {
      setLoading(true);
      // Pedimos el maximo permitido por el backend (50) para abarcar todo el curso
      ...
      <SheetContent className="sm:max-w-xl w-full sm:w-[600px] bg-[#f8fbff] border-l-[#d5e3f3] flex flex-col sm:p-8">
        <SheetHeader className="px-2">
          ...
          <SheetDescription className="text-sm font-medium text-[#6a8cb2] mt-1">
            Selecciona un estudiante para generar su boletín académico.
          </SheetDescription>
        </SheetHeader>

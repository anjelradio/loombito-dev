...
import BoletinesCourseItem from "./BoletinesCourseItem";
...
export default function BoletinesCourseWorkspace({ schoolId, courses }: Props) {
  return (
    <AccentCard
      variant="base"
      eyebrow="Cursos"
      title="Listado de cursos"
      description="Selecciona un curso para ver sus estudiantes y generar boletines."
      className="p-5"
    >
      {courses.length ? (
        <ScrollArea className="h-[65vh] pr-3">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 pb-1">
            {courses.map((course) => (
              <BoletinesCourseItem key={course.id} schoolId={schoolId} course={course} />
            ))}
          </div>
        </ScrollArea>
      ) : (
